...
export default function LobRuleComposer({
  ...
  const addTier = () =>
    setTiers((t) => [
      ...t,
      {
        ...
        max: "",
        payout: "",
        payoutType: payoutType === CompPayoutType.PERCENT_OF_PREMIUM ? CompPayoutType.PERCENT_OF_PREMIUM : CompPayoutType.FLAT_PER_APP,
      },
    ]);
  ...
  const summary = useMemo(() => {
    ...
    const names = selected.slice(0, 3).map((id) => products.find((p) => p.id === id)?.name).filter(Boolean);
  ...
  const tierSentence =
    tiers.length
      ? tiers
          ...
          .map((t) => {
            const range =
              t.min && t.max ? `${t.min}-${t.max}` : t.min && !t.max ? `${t.min}+` : !t.min && t.max ? `≤${t.max}` : "any";
            if (t.payoutType === CompPayoutType.PERCENT_OF_PREMIUM) {
              return `${range} premium pays ${t.payout || 0}%`;
            }
            return `${range} apps pay $${t.payout || 0}/app`;
          })
          .join("; ")
      : "";

  // Tier overlap helper for clarity
  const tierProblems = useMemo(() => {
    ...
    for (let i = 1; i < parsed.length; i++) {
      ...
      if (prev.max >= curr.min) {
        issues.push(`Tier ${prev.idx} (${prev.min}–${prev.max === Infinity ? "∞" : prev.max}) overlaps Tier ${curr.idx} (${curr.min}–${curr.max === Infinity ? "∞" : curr.max}).`);
      }
    }
    return issues;
  }, [tiers]);
  ...
  return (
    <div style={{ border: "1px dashed #cbd5e1", borderRadius: 12, padding: 14, background: "#f8fafc", display: "grid", gap: 12 }}>
      <div style={{ fontWeight: 800, marginBottom: 4 }}>Quick rule for {lobName}</div>
      <div style={{ fontSize: 12, color: "#475569", marginBottom: 6 }}>Guided steps: pick scope → payout/gate → tiers → review sentence.</div>
          ...
          >
            {products.map((p) => (
              <button
                ...
                onClick={() => !selected.includes(p.id) && setSelected((s) => [...s, p.id])}
                style={{
                  padding: "6px 8px",
                  borderRadius: 8,
      ...
      <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
        ...
        {tiers.map((t, idx) => (
          <div key={idx} style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(120px,1fr)) auto", gap: 8, alignItems: "center" }}>
            <input
              name="tierMin"
              value={t.min}
      ...
      <div style={{ marginTop: 12, padding: 10, borderRadius: 10, background: "#fff", border: "1px solid #e2e8f0" }}>
        ...
        {tierProblems.length ? (
          <div style={{ marginTop: 6, color: "#b45309", fontSize: 12, background: "#fef3c7", border: "1px solid #fcd34d", borderRadius: 8, padding: "6px 8px" }}>
