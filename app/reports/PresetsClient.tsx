...
export function PresetProductionOverview({ agencies }: { agencies: AgencyOption[] }) {
  ...
  const deriveMonthParam = () => {
    if (monthFilter) return monthFilter;
    if (granularity === "month") {
      const fromLabels = lob.labels?.[lob.labels.length - 1] || lob.labels?.[0];
      return fromLabels || format(startOfMonth(new Date()), "yyyy-MM");
    }
    const weekLabel = lob.labels?.[0];
    if (weekLabel) return format(startOfMonth(new Date(weekLabel)), "yyyy-MM");
    return format(startOfMonth(new Date()), "yyyy-MM");
  };
  const monthParam = deriveMonthParam();
  ...
  const deriveRangeParams = () => {
    ...
    if (lob.labels?.length) {
      const parseLabel = (label: string) => {
        if (!label) return undefined as Date | undefined;
        if (label.length === 7) return new Date(`${label}-01`);
        return new Date(label);
      };
      const first = parseLabel(lob.labels[0]);
      const last = parseLabel(lob.labels[lob.labels.length - 1]);
      const normalizedStart = first ? (granularity === "week" ? startOfWeek(first) : startOfMonth(first)) : undefined;
      const normalizedEnd = last ? (granularity === "week" ? endOfWeek(last) : endOfMonth(last)) : undefined;
      return {
        start: normalizedStart ? formatISODate(normalizedStart) : undefined,
        end: normalizedEnd ? formatISODate(normalizedEnd) : undefined,
      };
    }
  ...
  const renderChart = (id: string) => {
    ...
    if (id === "lobByAgencyGroup") {
      ...
      const handleBarClick = (params: any) => {
        ...
        qs.set("lob", lob);
        qs.set("agencies", agencyId);
        if (rangeSelection.start) qs.set("start", rangeSelection.start);
        if (rangeSelection.end) qs.set("end", rangeSelection.end);
        if (statusesParam) qs.set("statuses", statusesParam);
    ...
    if (id === "lobOverview") {
      ...
      const handleClick = (params: any) => {
        ...
        const agenciesParam = agencyId ? [agencyId] : selectedAgencyIds;
        const deriveMonth = () => {
          if (monthFilter) return monthFilter;
          const labelsSource = lob.labels && lob.labels.length ? lob.labels : trendFiltered.labels;
          if (labelsSource?.length) {
            const last = labelsSource[labelsSource.length - 1];
            if (last.length === 7) return last;
            const parsed = new Date(last);
            if (!isNaN(parsed.getTime())) return format(startOfMonth(parsed), "yyyy-MM");
          }
          return format(startOfMonth(new Date()), "yyyy-MM");
        };
        const month = deriveMonth();
        const qs = new URLSearchParams();
        qs.set("month", month);
        qs.set("lob", lobName);
        if (agenciesParam?.length) qs.set("agencies", agenciesParam.join(","));
        if (statusesParam) qs.set("statuses", statusesParam);
        window.location.href = `/sold-products?${qs.toString()}`;
      };
    ...
    if (id === "lobCards") {
            ...
            <Chart
              ...
              onEvents={{
                click: (params: any) => {
                  ...
                  if (!lobName) return;
                  const qs = new URLSearchParams();
                  if (currentRangeParams.start) qs.set("start", currentRangeParams.start);
                  if (currentRangeParams.end) qs.set("end", currentRangeParams.end);
                  if (!currentRangeParams.start && !currentRangeParams.end) qs.set("month", monthParam);
                  qs.set("lob", lobName);
                  if (selectedAgencyIds.length) qs.set("agencies", selectedAgencyIds.join(","));
                  if (statusesParam) qs.set("statuses", statusesParam);
                  window.location.href = `/sold-products?${qs.toString()}`;
                },
              }}
      ...
      const handleTrendPointClick = (params: any) => {
        ...
        const qs = new URLSearchParams();
        const derivedMonth = label ? format(startOfMonth(new Date(label)), "yyyy-MM") : monthParam;
        qs.set("month", derivedMonth);
        if (selectedAgencyIds.length) qs.set("agencies", selectedAgencyIds.join(","));
        if (statusesParam) qs.set("statuses", statusesParam);
        qs.set("lob", trendLobName);
        window.location.href = `/sold-products?${qs.toString()}`;
      };
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 10 }}>
            {lobOverviewCards.map((card) => {
              ...
              const handleCardClick = () => {
                ...
                qs.set("lob", card.lob);
                if (agencyFilter.length) qs.set("agencies", agencyFilter.join(","));
                if (statusesParam) qs.set("statuses", statusesParam);
                window.location.href = `/sold-products?${qs.toString()}`;
              };
