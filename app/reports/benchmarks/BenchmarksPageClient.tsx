              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ textAlign: "left", borderBottom: "1px solid #e5e7eb" }}>
                    ...
                    {hasCompare ? <th style={{ padding: 8 }}>Δ Premium Actual</th> : null}
                    <th style={{ padding: 8 }}>Prorated Premium Target</th>
                    {hasCompare ? <th style={{ padding: 8 }}>Δ Premium Target</th> : null}
                  ...
                  {breakdownRowEntries.map((row) => {
                    ...
                    return (
                      <tr key={rowKey} style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td style={{ padding: 8 }}>{fmtMoney(premiumActual)}</td>
                        {hasCompare ? <td style={{ padding: 8 }}>{premiumActualDelta}</td> : null}
                        <td style={{ padding: 8 }}>{fmtMoney(premiumTarget)}</td>
                        {hasCompare ? <td style={{ padding: 8 }}>{premiumTargetDelta}</td> : null}
                        <td style={{ padding: 8 }}>{fmtMoney(premiumDelta)}</td>
                ...
                {peopleRows.map((entry) => {
                  ...
                  return (
                    <tr key={personId} style={{ borderBottom: "1px solid #f3f4f6" }}>
                      ...
                      {hasCompare ? <td style={{ padding: 8 }}>{premiumActualDelta}</td> : null}
                      <td style={{ padding: 8 }}>{fmtMoney(premiumTarget)}</td>
                      <td style={{ padding: 8 }}>{fmtMoney(currentRow?.premiumDelta)}</td>
                      <td style={{ padding: 8 }}>{fmtPct(premiumPace)}</td>
        ...
        <div className="surface" style={{ padding: 12 }}>
          ...
          {mismatchLabels.length ? (
            <div style={{ color: "#6b7280", fontSize: 12, marginBottom: 8 }}>
              Note: Comparing against a snapshot with different filters:{" "}
              {mismatchLabels.map((label) => `[${label}]`).join(" ")}
            </div>
          ) : null}
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
