...
import { SourcePopover } from "@/app/sold-products/SourcePopover";
  ...
  export default async function SoldProductsPage({ searchParams }: { searchParams?: SearchParams }) {
    ...
    const marketingSourceOptions = Array.from(
      new Set(
        sortedRecent
          .map((r) => (r.household.marketingSource || "").trim())
          .filter(Boolean)
      )
    ).sort();
          ...
          {sortedRecent.length === 0 ? (
            ...
            <div>
              ...
              {householdOrder.map((hid) => {
                ...
                return (
                  <details key={hid} open={showRows} style={{ borderBottom: "1px solid #e5e7eb", position: "relative" }}>
                    <summary style={{ padding: "10px 16px 10px 44px", background: "#f8fafc", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", listStyle: "none", cursor: "pointer" }}>
                      <div style={{ fontWeight: 700 }}>
                    ...
                    <details style={{ position: "absolute", left: 8, top: 10, zIndex: 15 }}>
                      <summary
                        aria-label="Edit customer"
                        style={{ display: "inline-block", cursor: "pointer", listStyle: "none", padding: "3px 6px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", fontWeight: 700, fontSize: 12 }}
                      >
                        &#9998;
                      </summary>
                      <div
                    ...
                    {rows.map((r) => {
                      const displaySource = (r as any).marketingSourceOverride ?? r.household.marketingSource ?? "--";
                      return (
                          <div
                            key={r.id}
                            style={{
                          ...
                          >
                            ...
                            <div style={{ display: "flex", alignItems: "center" }}>
                              ...
                              >
                                ...
                                {renderReturnToInput()}
                                {statusStripOptions.map((option) => {
                                  const isSelected = r.status === option.value;
                                  return (
                                    <button
                                      key={option.value}
                                      type="submit"
                                      ...
                                      style={{
                                        padding: isSelected ? "6px 10px" : "6px 8px",
                                        borderRadius: 999,
                                    ...
                                    >
                                      {isSelected ? option.fullLabel : option.shortLabel}
                                    </button>
                                  );
                                })}
                              </form>
                            </div>
                            <SourcePopover label={displaySource}>
                              <div
                                style={{
                                  position: "absolute",
                                  right: 0,
                              ...
                              >
                                <form style={{ display: "grid", gap: 8 }}>
                                  <input type="hidden" name="householdId" value={r.householdId} />
                                  <input type="hidden" name="soldProductId" value={r.id} />
                                  {renderReturnToInput()}
                                  <label style={{ fontSize: 12, color: "#6b7280" }}>
                                    ...
                                    <select
                                      ...
                                      defaultValue={(r as any).marketingSourceOverride ?? r.household.marketingSource ?? ""}
                                      style={{ padding: 6, width: "100%", borderRadius: 8, border: "1px solid #d1d5db" }}
                                    >
                                      <option value="">--</option>
                                      {marketingSourceOptions.map((source) => (
                                  ...
                                  <div style={{ display: "flex", gap: 8 }}>
                                    <button
                                      type="submit"
                                      formAction={updatePolicySource}
                                      style={{ flex: 1, padding: "8px 10px", borderRadius: 8, border: "1px solid #d1d5db", background: "#f8fafc", color: "#111", fontWeight: 700 }}
                                    >
                                      Apply to This Policy
                                    </button>
                                    <button
                                      type="submit"
                                      formAction={updateHousehold}
                                      style={{ flex: 1, padding: "8px 10px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", color: "#111", fontWeight: 700 }}
                                    >
                                      Apply to All Policies in HH
                                    </button>
                                  </div>
                                </form>
                              </div>
